...
public:
	...
	enum ETechnique {
		...
		/// Unidirection path tracing with only an subset of
		EUnidirectionalMissing
	};
	 ...
	 */
	virtual void sampleSplats(const Point2i &offset, SplatList &list);
	...
	Float generateSeeds(size_t& sampleCount, size_t seedCount,
			...
			Float minNonZeroPathPerSeed = 0.f);
