...
public:
	 ...
	 * about the emission.
	 */
	virtual void handleNewParticle(const Ray ray,const Emitter &emitter);
...
protected:
	...
	ParticleTracer(int maxDepth, int rrDepth, bool emissionEvents, bool requestSampler = true);
	/// Protected constructor
	ParticleTracer(Stream *stream, InstanceManager *manager);
	/// Virtual destructor
	virtual ~ParticleTracer() { }
protected:
	bool m_requestSampler; //< Request the sampler when configure?
	ref<Scene> m_scene;
