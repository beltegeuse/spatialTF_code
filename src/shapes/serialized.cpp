	...
	SerializedMesh(const Properties &props) : TriMesh(props) {
		...
		*/
...
#ifdef MTS_NOSHADINGNORMAL
			// In our case, we don't need the surface loading
			// Indeed, m_faceNormals = true and during the configure of Trimesh::configure
			// The method Trimesh::computeNormals will be also called
			// and this method do:
			// if (m_faceNormals) {
	    // if (m_normals) {
	    //   delete[] m_normals;
	    ...
	    // } // ... etc.
			...
			if (m_normals) {
				for (size_t i=0; i<m_vertexCount; ++i) {
				  Normal temp = objectToWorld(m_normals[i]);
					m_normals[i] = normalize(temp);
				}
			}
#endif
		}
