...
public:
	...
	int sampleComponent(const BSDFSamplingRecord &bRec, Float &pdf,
	                    Point2 &_sample, const Float roughtConst) const {
	  NotImplementedError("roughtCoating::sampleComponent, level error");
	  ...
	  /* Find the probability of sampling the diffuse component */
    Float probSpecular = 1 - m_roughTransmittance->eval(std::abs(Frame::cosTheta(bRec.wi)), alpha);
    ...
    if (sample.x < probSpecular) {
      ...
      bRec2.wi = refractTo(EInterior, bRec.wi);

      // === Sample the comp from the neasted bsdf
      int compSampled = m_nested->sampleComponent(bRec2, pdf, sample, roughtConst);

      // === Scale the PDF of choosing the component
      pdf *= (1-probSpecular);
