...
public:
	...
	int sampleComponent(const BSDFSamplingRecord &bRec, Float &pdf,
	    ...
	    // === Iterate on all component to know if we need
	    // to have different decisions
	    std::vector<std::pair<int, int> >::const_iterator it = m_indices.begin();
	    int nbBouncing = 0;
	    int nbNoBouncing = 0;
	    while(it != m_indices.end()) {
	      int bsdfIndex = (*it).first;
	      int component = (*it).second;
	      if(m_bsdfs[bsdfIndex]->getRoughness(bRec.its, component) > roughtConst) {
	        nbNoBouncing++;
	      } else {
	        nbBouncing++;
	      }
	    ...
	    // Two cases now:
	    //  - Same decision: All bouncing or not => Sample all components
	    //  - Different solution => Just select randomly one of the component

	    if(nbBouncing == 0 || nbNoBouncing == 0) {
	      pdf = 1;
	      return -1; // select all component
	    } else {
	      size_t entry = m_pdf.sampleReuse(sample.x);
	      int comp = m_offsets[entry]+m_bsdfs[entry]->sampleComponent(bRec, pdf, sample, roughtConst);
	      pdf *= m_pdf[entry];
	      return comp;
	    }
	  }
