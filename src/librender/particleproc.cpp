...
ParticleTracer::ParticleTracer(int maxDepth, int rrDepth, bool emissionEvents, bool requestSampler)
	: m_maxDepth(maxDepth), m_rrDepth(rrDepth), m_emissionEvents(emissionEvents), m_requestSampler(requestSampler) { }
...
void ParticleTracer::process(const WorkUnit *workUnit, WorkResult *workResult,
	...
	for (size_t index = range->getRangeStart(); index <= (range->getRangeEnd()+m_excess) && !stop; ++index) {
		...
		while (!throughput.isZero() && (depth <= m_maxDepth || m_maxDepth < 0)) {
			Spectrum oldThroughput = throughput;
		  m_scene->rayIntersectAll(ray, its);
			...
			if (medium && medium->sampleDistance(Ray(ray, 0, its.t), mRec, m_sampler)) {
				...
				*/
				...
				handleSurfaceInteraction(depth, nullInteractions, delta, its, medium, throughput*power, wo);
				...
				Float tMult = throughput.average()/oldThroughput.average();
				if(tMult > 1.10) { // Super sensitive to IS problems
				  // Print an warning message if IS increase the throughtput more than %10
				  Log(EWarn, "IS issue (%f): %s", tMult, bsdf->toString().c_str());
				}
...
void ParticleTracer::handleNewParticle(const Ray ray,const Emitter &emitter) { }
