...
void Scene::recomputeWeightEmitterFlux() {
  // First clear all previous weights
  m_emitters.ensureUnique(); //< Just to be sure
  m_emitterPDF.clear();
  ...
  for (ref_vector<Emitter>::iterator it = m_emitters.begin();
      ...
      m_emitterPDF.append(it->get()->getPower());
	  it->get()->setSamplingWeight(it->get()->getPower());
  }

  m_emitterPDF.normalize();

  Log(EInfo, "Emitter rescale pdfs:");
  int iEmitter = 0;
  for (ref_vector<Emitter>::iterator it = m_emitters.begin();
            it != m_emitters.end(); ++it) {
    Log(EInfo, " - Emitter %s: %f", (*it)->getID().c_str(), m_emitterPDF[iEmitter]);
    iEmitter+=1;
  }
}
