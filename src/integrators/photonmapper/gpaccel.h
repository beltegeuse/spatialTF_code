...
public:
	...
	SAHTree(std::vector<std::vector<PixelData<T> > >& gps):
		m_nodesGP(0), m_nGPS(0)
	{
		// Compute number Gps
		for (int blockIdx = 0; blockIdx<(int) gps.size(); ++blockIdx) {
			std::vector<PixelData<T> > &gatherPoints = gps[blockIdx];
			for (size_t i=0; i<gatherPoints.size(); ++i) {
				PixelData<T> &gpList = gatherPoints[i];
				typename PixelData<T>::iterator gp = gpList.begin();
			...
			for (size_t i=0; i<gatherPoints.size(); ++i) {
				...
				for (; gp != gpList.end(); ++gp ) {
					if(gp->its.isValid() && gp->depth != -1) {
						...
						Float radius = node.gp->points->radius;
						sumRadii += radius;
						node.aabb = AABB(
...
public:
	...
	MedianTree(std::vector<std::vector<PixelData<T> > >& gps, bool build, Float radiiScale = 1.f):
	m_nodesGP(0), m_nGPS(0)
	{
		// Compute number Gps
		for (int blockIdx = 0; blockIdx<(int) gps.size(); ++blockIdx) {
	...
	void expendGP(std::vector<std::vector<PixelData<T> > >& gps, Float radiiScale = 1.f) {
	    int oldNbGPS = m_nGPS;
...
public:
	...
	GPAccelPointKD(std::vector<std::vector<PixelData<T> > >& gps):
		m_kdtree(0, GatherTree::ELeftBalanced)
	{
		...
		for (int blockIdx = 0; blockIdx<(int) gps.size(); ++blockIdx) {
			...
			for (size_t i=0; i<gatherPoints.size(); ++i) {
				...
				for (; gp != gpList.end(); ++gp ) {
					if(gp->its.isValid() && gp.depth != -1) {
						m_maxRadius = std::max(gp->radius, m_maxRadius);
						gatherCount++;
					}
				}
			}
		...
		for (int blockIdx = 0; blockIdx<(int) gps.size(); ++blockIdx) {
            ...
            for (size_t i=0; i<gatherPoints.size(); ++i) {
                ...
                for (; gp != gpList.end(); ++gp ) {
                    if(gp->its.isValid() && gp->depth != -1) {
                        gatherCount++;
                        m_maxRadius = std::max(gpList.radius, m_maxRadius); // Data attached to the  pixel
                    }
                }
            }
        }
...
public:
	...
	GPAccelBVH(std::vector<std::vector<PixelData<T> > >& gps):
		m_kdtree(gps)
	{
...
public:
	...
	GPAccelOctree(std::vector<std::vector<PixelData<T> > >& gps):
		m_octree(0)
	{
		// === Compute the AABB
		AABB aabbGPS;
...
struct GPAccel
{
    // Inside the paper, we use SAH tree for GPs acceleration structure.
    // Other data structure are available and can be more efficient for some type of scene
