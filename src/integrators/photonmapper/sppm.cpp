...
public:
	...
	void distributedRTPass(Scene *scene, std::vector<SerializableObject *> &samplers) {
		...
		#endif
		for (int i=0; i<(int) m_gatherBlocks.size(); ++i) { // For all gatherpoints blocks
			...
			// === Get the current offset for the pixels
				...
				for (int xofsInt = 0; xofsInt < blockSize; ++xofsInt) {
					...
					while (true) { // --- Bounce ...
						if (scene->rayIntersect(ray, gatherPoint.its)) {
							...
							// === Ok
#if SPPM_MOD
							bool smoothFunction = bsdf->getType() & BSDF::ESmooth;
							if (smoothFunction ||
	...
	void photonMapPass(int it, RenderQueue *queue, const RenderJob *job,
		...
		// === Create a photon map of shooted particules
		...
		// === Get info and update statistics
		Log(EInfo, "Gathering ..");
		m_totalEmitted += proc->getShotParticles();
		m_totalPhotons += photonMap->size();

		// === Gather the energy for gather points by query the photon map.
		film->clear();
		#if defined(MTS_OPENMP)
			#pragma omp parallel for schedule(dynamic)
		#endif
