...
class MIPathTracer : public MonteCarloIntegrator {
 public:
  MIPathTracer(const Properties &props)
    ...
    m_roughness = props.getFloat("bounceRoughness");
  }
    ...
    while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
      ...
      if (rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance
        ...
        if (!value.isZero()) {
          ...
          ////////////////////////////////////////////////
          // Now we have to check the bouncing procedure
          // If it's a smooth BSDF, just we cancel the connection procedure
          // Otherwise, we continue the connection
          Float pdfComponent = 0.f;
          Point2 randomBSDFSample = rRec.nextSample2D();
          ...
          // Query the roughtness to know the bounce decision
          // if the componentSelected is all, the bounce decision
          // will be the same for all component, so take one of them
          // to query the roughtness
          int componentQuery = (
              componentSelected == -1 ? 0 : componentSelected);
          bool roughtFunction = bsdf->getRoughness(its,
                                                   componentQuery)
              >= m_roughness && (!(bRec.sampledType & BSDF::EDelta));

          if(!roughtFunction) {
            // In case of connecting, only on the component selected
            ...
            if (!bsdfVal.isZero()
              ...
              Float weight = miWeight(dRec.pdf*pdfComponent, bsdfPdf* pdfComponent);
              Li += throughput * value * bsdfVal * weight / pdfComponent;
            }
          }
        }
      }
      ...
      bool roughtFunction = bsdf->getRoughness(its, componentQuery)
          >= m_roughness && (!(bRec.sampledType & BSDF::EDelta));
      if(roughtFunction) {
        // Normally we put an gather point here
        // So this is just break the bouncing procedure.
        // STOP here
        return Li;
      }

      // We didn't put an gather point here.
      // So, we just continue to create an path
      // By bouncing only on the component selected
      bRec.component = componentSelected;

      /* Sample BSDF * cos(theta) */
      Float bsdfPdf;
      Spectrum bsdfWeight = bsdf->sample(bRec, bsdfPdf, randomBSDFSample) / pdfComponent; // dividing by PDF of selecting one component
      ...
      if (hitEmitter
        ...
        Li += throughput * value * miWeight(bsdfPdf * pdfComponent, lumPdf * pdfComponent);
      }
  ...
  std::string toString() const {
    ...
    oss << "MIPathTracer[" << endl << "  maxDepth = " << m_maxDepth << ","
        << endl << "  rrDepth = " << m_rrDepth << "," << endl
