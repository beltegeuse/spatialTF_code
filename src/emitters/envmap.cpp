...
public:
	...
	void pdfRay(const Ray &ray, Float & pdfEmit, Float & pdfDirect, Float & cosThetaLight) const {
		const Transform &trafo = m_worldTransform->eval(ray.time);
		Float pdfDirection = internalPdfDirection(trafo.inverse()(-ray.d));

		pdfEmit = pdfDirection / (M_PI * m_geoBSphere.radius * m_geoBSphere.radius);
		pdfDirect = pdfDirection;
		cosThetaLight = 1.0f;
	}
